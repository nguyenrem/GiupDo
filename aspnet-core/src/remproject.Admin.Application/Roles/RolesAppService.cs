...
namespace remproject.Admin.Roles
{
        ...
        CreateUpdateRoleDto>, IRolesAppService
    {
        ...
        public async override Task<RoleDto> CreateAsync(CreateUpdateRoleDto input)
        {
            ...
            if (isNameExisted)
            {
                throw new BusinessException(remprojectDomainErrorCodes.RoleNameAlreadyExists)
        ...
        public async Task<GetPermissionListResultDto> GetPermissionsAsync(string providerName, string providerKey)
        {
            ...
            foreach (var group in PermissionDefinitionManager.GetGroups().Where(x => x.Name.StartsWith("AbpIdentity") || x.Name.StartsWith("remproAdmin")))
            {
